...
class MatchGame extends Component {
  ...
  componentDidMount() {
    this.startTimer = setInterval(this.changeTimer, 1000)
  }
  ...
  selectImage = id => {
    ...
    this.setState(prevState => ({
      score:
        id === prevState.selectedImageId
          ? prevState.score + 1
          : prevState.score,
      selectedImageId: imagesList[randomNumber].id,
      running: prevState.selectedImageId === id,
    }))
  }
  ...
  render() {
    const {score, timer, selectedTabId, selectedImageId, running} = this.state
    if (timer === 0 || !running) {
      clearInterval(this.startTimer)
    }
    ...
    const filteredTabsList = tabsList.map(eachTab => {
      if (eachTab.tabId === selectedTabId) {
        return {...eachTab, selected: true}
      }
      return {...eachTab, selected: false}
    })

    const selectedImageItem = imagesList.filter(
      eachItem => eachItem.id === selectedImageId,
    )[0]
